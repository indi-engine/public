...
class Indi_View_Helper_SeoTDK {
		...
		if (Indi::trail()->row) {
            if (Indi::trail()->section->entityId) {
                if (is_object($this->exclusion)) {
                    ...
                    if ($this->exclusion = $exclusionM->fetchRow('`entityId`="' . Indi::trail()->section->entityId . '" AND `identifier` = "' . Indi::trail()->row->id . '" AND `toggle` = "y"')){
                        return $this->exclusion->{'seo' . ucfirst($what)};
                    } else {
            ...
            if (Indi::view()->row->useSystemSeoSolution == 'n') {
				...
				$parts = Indi::model('Seo'. ucfirst($what))->fetchAll('`fsection2factionId`="' . Indi::trail()->section2action->id . '"', '`move`');
						...
						if ($part->type == 's') {
							...
							if ($part->where == 'c') {
								if (Indi::trail($part->stepsUp)->section->entityId == $part->entityId) {
									...
									$siblingRow[Indi::trail()->model->table() . 'Id'] = Indi::view()->row;
								...
								$pkn = $model->table() . 'Id';
								$pkv = $siblingRow[$siblingModel->table() . 'Id']->$pkn;
								$row = $model->fetchRow('`id` = "' . $pkv . '"');
								if ($part->foreign('fieldId')->relation) {
									$title[] = $part->prefix . $row->foreign($part->foreign('fieldId')->alias)->title . $part->postfix;
								} else {
			...
			if( ! is_array($this->cr)) {
				...
				for($i = 0; $i < count(Indi_Trail_Front::$items); $i++){
					$item = Indi::trail($i);
					if ($item->row) $this->cr[$entity->fetchRow('`table` = "' . $item->model->table() . '"')->id] = $item->row;
				}
			}
	...
	function constructSeoForRowsetActions($what, $parentId = 0){
		$parts = Indi::model('Seo'. ucfirst($what))->fetchAll('`seo'. ucfirst($what) . 'Id` = "' . $parentId . '" AND `fsection2factionId`="' . Indi::trail()->section2action->id . '"', '`move`');
		$parts->foreign('fieldId,sibling');
		static $siblingRow;
		if (!is_array($siblingRow)) $siblingRow = array();
		if ($parts->count()) {
